...
//                    exit(EXIT_FAILURE);  // Push the result of the division (in rax)
//    			}
...
void operator()(const NodeBinExprDiv* div) const
{
    gen.gen_expr(div->rhs);  // Push RHS (denominator)
    gen.gen_expr(div->lhs);  // Push LHS (numerator)
    gen.pop("rbx");          // RHS → rbx
    gen.pop("rax");          // LHS → rax

    const std::string label_safe = gen.create_label();
    const std::string label_div_zero = gen.create_label();

    // Check if divisor is zero
    gen.m_output << "    cmp rax, 0\n";
    gen.m_output << "    je " << label_div_zero << "\n";

    // Safe division
    gen.m_output << "    xor rdx, rdx\n";      // Clear rdx before division
    gen.m_output << "    div rbx\n";
    gen.push("rax");
    gen.m_output << "    jmp " << label_safe << "\n";

    // Division by zero handler
    gen.m_output << label_div_zero << ":\n";
    gen.m_output << "    mov rax, 1\n";        // write syscall
    gen.m_output << "    mov rdi, 1\n";        // stdout
    gen.m_output << "    mov rsi, div_zero_msg\n";
    gen.m_output << "    mov rdx, 23\n";       // length of message
    gen.m_output << "    syscall\n";
    gen.m_output << "    mov rax, 60\n";       // exit syscall
    gen.m_output << "    mov rdi, 1\n";        // error code 1
    gen.m_output << "    syscall\n";

    gen.m_output << label_safe << ":\n";
}
    ...
    [[nodiscard]] std::string gen_prog()
    {
		...
		m_output << "div_zero_msg db \"Division by zero error\", 10\n";
